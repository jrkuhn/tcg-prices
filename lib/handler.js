...
function getPricesSku(skus){
  ...
  var url = "http://api.tcgplayer.com/v1.17.0/pricing/sku/"+skus;
  var options = { 
    method: 'GET',
    url: url,
    headers: 
...
function deliverPricesNm(name, sort) {
  return new Promise(resolve => {
    ...
    .then(function(skuResp) { //filter NM sku's
      ...
      results.prices = nmSkus.results;

      return getDetails(prodIds);
    })
    .then(function(detailsResp) { //add product details to matching sku's
      var cardIndex = {};
      var cardDetails = JSON.parse(detailsResp);
      ...
      var skus = skuIds.join(",");
      return getPricesSku(skus);
    })
    .then(function(priceResp) {
      var priceIndex = {}, skuPrices = JSON.parse(priceResp);

      //index prices by skuid
      skuPrices.results.forEach(function(price, i) {
        priceIndex[price.skuId] = price;
      });
      console.log(priceIndex);

      //add price data to each sku result
      results.prices.forEach(function(price, i) {
        var id = results.prices[i].skuId;
        if(priceIndex[id]){
          results.prices[i].lowPrice = priceIndex[id].lowPrice;
          results.prices[i].marketPrice = priceIndex[id].marketPrice;
        }
      });
